...
def loadData(names):
    if names == 'river':
        data_path = os.path.join(r'data/river')
        ...
        labels[labels == 255] = 1
        print("dataset is river")
    
    if names == 'farm':
        data_path = os.path.join(r'data/farmland')
        data1 = sio.loadmat(os.path.join(data_path, 'China_Change_Dataset.mat'))['T1']
        data2 = sio.loadmat(os.path.join(data_path, 'China_Change_Dataset.mat'))['T2']
        labels = sio.loadmat(os.path.join(data_path, 'China_Change_Dataset.mat'))['Binary']
        print("dataset is farm")


    if names == 'Hermiston':
        data_path = os.path.join(r'data/Hermiston')
       
        data1 = sio.loadmat(os.path.join(data_path, 'hermiston2004.mat'))['img_rhyde']
        data2 = sio.loadmat(os.path.join(data_path, 'hermiston2004.mat'))['img_rhyde']
        labels = sio.loadmat(os.path.join(data_path, 'rdChangesHermiston_5classes.mat'))['gt5clasesHermiston']
        print("dataset is Hermiston")
   

    if names == 'bayArea':
      
        data_path = os.path.join(r'data/bayArea')
        ...
        labels = sio.loadmat(os.path.join(data_path, 'bayArea_gtChanges2.mat'))['HypeRvieW']
        uc_position = np.array(np.where(labels == 2)).transpose(1, 0)
        c_position = np.array(np.where(labels == 1)).transpose(1, 0)
        print((uc_position.shape[0], c_position.shape[0]))
        print("data is BayArea")

    if names == 'Barbara':
      
        data_path = os.path.join(r'./data/santaBarbara')
        data1 = sio.loadmat(os.path.join(data_path, 'barbara_2013.mat'))['HypeRvieW']
        data2 = sio.loadmat(os.path.join(data_path, 'barbara_2014.mat'))['HypeRvieW']
        labels = sio.loadmat(os.path.join(data_path, 'barbara_gtChanges.mat'))['HypeRvieW']
        uc_position = np.array(np.where(labels == 2)).transpose(1, 0)
        c_position = np.array(np.where(labels == 1)).transpose(1, 0)
        print((uc_position.shape[0], c_position.shape[0]))
        print("data is BayArea")
...
def set_train_sample_for_bayorbar(x, y, pos, neg):
    y = np.where(y == 0, 3, y)
    y = np.where(y == 2, 0, y)
...
def reports(test_iter, net, device):
    ...
    with torch.no_grad():
        for step, (x1, x2, x3, x4, y) in enumerate(test_iter):
            ...
            size = y_hat.numel()
            y_hat = binary(y_hat, th=0.8, bathsize=size) # threshold value （0.3,0.5,0.7,0.8,0.9,0.99）
            y_hat = y_hat.cpu()
            y_hat = y_hat.tolist()
            y_pred.extend(y_hat)
            y_test.extend(y.cpu())
    ...
    return classification, confusion, oa * 100, aa * 100, kappa * 100, f1 * 100
...
def generater(X_1, X_2, Y, batchsize, windowSize):
    ...
    X1_train, X1_test, y_train, y_test = set_train_sample(alldataX1, alldataY,     pos = 980 ,neg = 1960 )
    ...
    X2_train, X2_test, y_train, y_test = set_train_sample(alldataX2, alldataY,   pos = 980 ,neg = 1960)
